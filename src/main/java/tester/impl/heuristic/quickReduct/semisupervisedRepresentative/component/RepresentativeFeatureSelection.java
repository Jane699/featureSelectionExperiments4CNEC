...
/**
 * Representative Feature Selection for <strong>Semi-supervised Representative Feature Selection</strong>. 
 * This procedure contains 3 ProcedureComponents: 
 * <li>
 * 	<strong>Representative selection controller</strong>
 * 	<p>Control loop over features in each generated sub-graph of Directed Acyclic Graph(DAG), and select the
 * 		one with max. F1-Rel value as the representative of the sub-graph(group) according to the original 
 * 		article <a href="https://linkinghub.elsevier.com/retrieve/pii/S0031320316302242">"An efficient 
 * 		semi-supervised representatives feature selection algorithm based on information theory"</a>, and 
 * 		finally as an attribute of the reduct.
 * </li>
 * <li>
 * 	<strong>Subgraph generation</strong>
 * 	<p>Generate sub-graphs of the Directed Acyclic Graph(DAG). Nodes connected to each other are at the same
 * 		sub-graph(group). Using Deep First Search(DFS) strategy when searching.
 * </li>
 * <li>
 * 	<strong>Representative selection</strong>
 * 	<p>Select the select nodes with max. F1-Rel value as the representatives of the sub-graphs(groups).
 * </li>
 * 
 * @author Benjamin_L
 */
...
//						if (logOn)	log.info("1. "+component.getDescription());
					...
					false, (component, parameters) -> {
						...
						Collection<Collection<Integer>> subGraphs = (Collection<Collection<Integer>>) comp1.exec();
...
//						if (logOn)	log.info("2. "+component.getDescription());
					...
					false, (component, parameters) -> {
						...
						Collection<DirectedAcyclicGraghWeightedEdge<Integer, Double>>[] dagEdges = 
								(Collection<DirectedAcyclicGraghWeightedEdge<Integer, Double>>[]) parameters[p++];
						/* ------------------------------------------------------------------------------ */
						TimerUtils.timeStart((TimeCounted) component);
						/* ------------------------------------------------------------------------------ */
					...
					(component, subGraphs) -> {
						/* ------------------------------------------------------------------------------ */
						getParameters().setNonRootParameter("subGraphs", subGraphs);
						if (logOn) {
							log.info(LoggerUtil.spaceFormat(1, "x {} sub-graph(s) in total."), subGraphs.size());
//							int index = 1;
//							for (Collection<Integer> subgraph: subGraphs) {
...
//						if (logOn)	log.info("3. "+component.getDescription());
					...
					false, (component, parameters) -> {
						...
						Collection<Integer> subgraph = (Collection<Integer>) parameters[p++];
						/* ------------------------------------------------------------------------------ */
						TimerUtils.timeStart((TimeCounted) component);
						/* ------------------------------------------------------------------------------ */
