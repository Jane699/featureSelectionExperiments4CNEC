package featureSelection.statistics.recorder;
...
import basic.utils.ExcelUtils;
import basic.utils.LoggerUtil;
import featureSelection.statistics.Constants;
import featureSelection.utils.StatisticsUtils;
...
public class ReportRecorder {
	...
	/**
	 * Save a reports of {@link ReportMapGenerated} with <code>component.getDescription()</code> as 
	 * key, report {@link Map} as value.
	 * 
	 * <p> report : {
	 ...
	 * @param report
	 * 		A report Map of a {@link ReportMapGenerated} by calling {@link ReportMapGenerated#getReport()}.
	 * @throws IOException if exceptions occur when writing report files.
	 */
	private static void saveAReport(File destFile, String[] reportItemOrder,
	 ...
	 * @throws IOException if exceptions occur when writing report files.
	 */
	public static void saveReports(File path, String datasetName, int timesIndex, int attrOrderIndex,
	...
	public static void saveSubReports(File reportBaseDir, ProcedureContainer<?> container) throws IOException {
		if (container instanceof ReportMapGenerated) {
			@SuppressWarnings("unchecked")
			ReportMapGenerated<String, Map<String, Object>> report = (ReportMapGenerated<String, Map<String, Object>>) container;
	...
	private static String commonReportPath(String datasetName, String algorithmName) {
		return Constants.Report.REPORT_FILE_BASE_FOLDER + File.separator+
	 ...
	 */
	private static String[] reportTitles(Map<String, Map<String, Object>> report, 
										String...priorityTitleOrder
	) {
		String firstColumn = Constants.Report.REPORT_FIRST_COLUMN_DESCRIPTION;
	...
	private static Collection<String[]> record(String[] titles, String[] reportItemOrder,
												Map<String, Map<String, Object>> report
	) {
		Object value;
		String[] data;
	...
	private static Object executionTimeFormatFilter(String title, Object value) {
		if (tester.frame.statistics.heuristic.Constants.REPORT_EXECUTION_TIME.equals(title)) {
	...
	public static void saveAverageExecTimeReports(File path, String datasetName, String algorithmName,
	...
	private static void loadAverageItemOrder(Map<String, String[]> orders, ProcedureContainer<?>...containers){
		String[] existsTitle;
	...
	private static Map<String, Map<String, Map<String, Object>>> averageExecuteTimeReport(
		...
		for (Map.Entry<String, Map<String, Map<String, Object>>> collectEntry: collect.entrySet()) {
			for (Map<String, Object> dataMap: collectEntry.getValue().values()) {
				execTimeInfo = (long[]) dataMap.get(tester.frame
														.statistics
														.heuristic
														.Constants
	...
	private static void loadSumExecuteTimeInfo(
			Map<String, Map<String, Object>> collectMap, Map<String, Map<String, Object>> reportMap
	) {
		long[] timeInfo = null;
		...
		for (String componentDesc : reportMap.keySet()) {
			...
			for (String key: reportData.keySet()) {
				if (tester.frame
						...
						.heuristic
						.Constants
						.REPORT_EXECUTION_TIME.equals(key)
				){
