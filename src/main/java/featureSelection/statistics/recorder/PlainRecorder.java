...
public class PlainRecorder {
	public final static String FILE_NAME_ALL_RECORDS = "All Records.csv";
	public final static String FILE_NAME_SUMMARY = "Summary.csv";
	public final static String FILE_NAME_MAIN_REDUCT = "MainReduct.csv";
	
	public static void saveAFileRecords(File path, List<RecordFieldInfo> titles, PlainRecord...record) 
			throws IOException 
	{
		saveAFileRecords(path, FILE_NAME_ALL_RECORDS, titles, record);
	}
	
	public static void saveAFileRecords(File path, String fileName, Collection<RecordFieldInfo> titles, PlainRecord...record) 
			throws IOException 
	{
	...
	public static void saveOrderDetailAverageFileRecords(File path, List<RecordFieldInfo> titles, PlainRecord...record)
			throws IOException 
	{
		if (!path.exists())	path.mkdirs();
	...
	public static void saveAlgorithmFileAverage(File path, List<RecordFieldInfo> titles, PlainRecord...record) 
			throws IOException 
	{
		if (!path.exists())	path.mkdirs();
		File aveFile = new File(path, "Average Records(Comprehensive).csv");
	...
	private static String[] averageRecord(List<RecordFieldInfo> titles, PlainRecord[] records) {
	...
	private static String[] record(Collection<RecordFieldInfo> titles, PlainRecord record) {
		...
		for (RecordFieldInfo title: titles)	lineData[i++] = StatisticsUtils.record(title, record, 100, 2);
		return lineData;
	}
}